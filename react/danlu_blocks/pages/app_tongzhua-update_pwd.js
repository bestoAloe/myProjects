...
const action42 = modalInstance
const action1 = actions['onCreateState'] = function (store, event) {
  const state = store.state
  const action31 = state.password = ("")
...
const action4 = actions['func1'] = async function (store, event) {
  const state = store.state
  const action5 = (() => {
    if (!(v => v && typeof v === 'string')(state.password)) return "请填写旧密码！" // pwd
  ...
  const action10 = (await dw.fetch("post/psiteuser/set_password", {"password": state.password, "password1": state.password1, "password2": state.password2})).data
  
        if ((action10.status || !action39.correct) === ("error")) {
          const action13message = (action10.message || "操作错误！")
  ...
  let msg = ("密码修改成功")
  const action40 = msg
  const action41 = await (async (store, event) => {
if(modalInstance){
  console.log('step1')
...
const action36 = actions['getModalInstance'] = function (store, event) {
  const state = store.state
  const action37 = modalInstance = (event.data)
  const action38 = console.log('[getModalInstance#action38] 打印(modalInstance)', modalInstance)
}
...
const action34 = actions['func22'] = function (store, event) {
  const state = store.state
  const action35 = dw.app.goToPappUrl("reset_pwd", {}, {redirectTo: false})
}
...
const pageGroupSlug = 'app_tongzhua-update_pwd';
...
{!!(slide.components.ViewIf1.ctx_map.value) /* ViewIf1 */ && (<><View /* View16 */   style={styles.xdw3m0}><View /* View7 */   style={styles.xdw3lr}><View /* View8 */   style={styles.xdw3ls}><View /* View9 */   style={styles.xdw3lr}><Text /* Text15 */ style={styles.xdw3lq} >{slide.components.Text15.ctx_map.text}</Text></View></View></View>
<View /* View5 */   style={[styles.xdw3lv, (state.platform === 'rnapp' ? 
    state.focus_input === 'password' ? {
        borderBottomWidth: 1,
    ...
    : state.focus_input === 'password' ? {
        ...
        borderBottomWidth: "1px",
        borderBottomColor: '#E8E8E8',
    })]}><XbenchcInput /* NativeInput2 */ style={styles.xdw3lt} value={state.password} placeholder={slide.components.NativeInput2.ctx_map.placeholder} type={slide.components.NativeInput2.ctx_map.type} password={slide.components.NativeInput2.ctx_map.password}  onChange={getEvent('NativeInput2:onChange:func3:', {eventData: "password"}, bagForKeyValue, bagForIndex)} onFocus={getEvent('NativeInput2:onFocus:func21:', {eventData: "password"}, bagForKeyValue, bagForIndex)} />
<Text /* Text21 */ style={styles.xdw3lu} >{slide.components.Text21.ctx_map.text}</Text></View>
<View /* View17 */   style={[styles.xdw3lw, (state.platform === 'rnapp' ? 
    ...
    : state.focus_input === 'password1' ? {
        ...
        borderBottomWidth: "1px",
        borderBottomColor: '#E8E8E8',
    })]}><XbenchcInput /* NativeInput3 */ style={styles.xdw3lt} value={state.password1} placeholder={slide.components.NativeInput3.ctx_map.placeholder} type={slide.components.NativeInput3.ctx_map.type} password={slide.components.NativeInput3.ctx_map.password}  onChange={getEvent('NativeInput3:onChange:func3:', {eventData: "password1"}, bagForKeyValue, bagForIndex)} onFocus={getEvent('NativeInput3:onFocus:func21:', {eventData: "password1"}, bagForKeyValue, bagForIndex)} /></View>
<View /* View4 */   style={[styles.xdw3lw, (state.platform === 'rnapp' ? 
    ...
    : state.focus_input === 'password2' ? {
        ...
        borderBottomWidth: "1px",
        borderBottomColor: '#E8E8E8',
    })]}><XbenchcInput /* NativeInput1 */ style={styles.xdw3lt} value={state.password2} placeholder={slide.components.NativeInput1.ctx_map.placeholder} type={slide.components.NativeInput1.ctx_map.type} password={slide.components.NativeInput1.ctx_map.password}  onChange={getEvent('NativeInput1:onChange:func3:', {eventData: "password2"}, bagForKeyValue, bagForIndex)} onFocus={getEvent('NativeInput1:onFocus:func21:', {eventData: "password2"}, bagForKeyValue, bagForIndex)} /></View>
<View /* View2 */   style={styles.xdw3lz}><XbenchcView /* View3 */   viewStyle={[styles.xdw3ly, (!(state.password1 && state.password2 && state.password) ? {
    backgroundColor: "#FFF1C6",
} : {
    backgroundColor: "#FFD142",
})]} onClick={getEvent('View3:onClick:func1:', {}, bagForKeyValue, bagForIndex)}><Text /* Text13 */ style={styles.xdw3lx} >{slide.components.Text13.ctx_map.text}</Text></XbenchcView></View>
