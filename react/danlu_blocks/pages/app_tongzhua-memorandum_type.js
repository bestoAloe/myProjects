...
const action3 = actions['func1'] = async function (store, event) {
  const state = store.state
  const action4 = await dw.app.run("auto_group_category.list", {"aggs": {}, "limit": 100, "offset": 0, "filters": {data__type:"todo"}, "order_by": ["data__order__float"]})
  const action7 = store.commit('setState', {["state.items"]: action4.objects})
}

const action8 = actions['func2'] = function (store, event) {
  const state = store.state
  const action9 = dw.app.goToPappUrl("memorandum_create", {"uuid": event.context.eventData.uuid}, {redirectTo: false})
}
...
const pageGroupSlug = 'app_tongzhua-memorandum_type';
...
<XbenchcScrollView /* ScrollView1 */ style={styles.xdw2hr} scrollX={ false } scrollY={ true } ><View /* View1 */   style={styles.xdw2hq}>{(state.items).map((bagForItem, bagForIndex) => { const bagForKeyValue = 'ViewFor1' + ':' + String(bagForItem['id']); return <ViewForItem /* ViewFor1 */ key={bagForKeyValue} bagForItem={bagForItem} bagForIndex={bagForIndex}><XbenchcView /* View2 */   viewStyle={styles.xdw2hp} onClick={getEvent('View2:onClick:func2:', {eventData: bagForItem}, bagForKeyValue, bagForIndex)}><XbenchcImage /* Image1 */ style={styles.xdw2hn} source={dwtools.qiniu(bagForItem.data.image, slide.components.Image1.ctx_map.fop)}  />
<Text /* Text1 */ style={styles.xdw2ho} >{bagForItem.data.name}</Text></XbenchcView></ViewForItem> })}</View></XbenchcScrollView>
    </ErrorBoundary>
  )
}
