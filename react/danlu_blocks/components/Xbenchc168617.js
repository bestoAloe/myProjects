...
const configFactory = ((dw) => { return {
  ...
  methods: {
    handleChange (event) {
      if (String(event.detail.value) === String(parseFloat(event.detail.value))) {
        this.triggerEvent('input', parseFloat(event.detail.value))
      } else {
        this.triggerEvent('input', event.detail.value)
      }
    },
    handleMinus () {
      this.triggerEvent('input', parseFloat(this.data.value) - 1)
    },
    handlePlus () {
      this.triggerEvent('input', parseFloat(this.data.value) + 1)
    },
...
const render = (_props, _instance, _xc, children) => {
  const { eventData, eventData2, value, __0 } = _props
  const { __1 } = _instance.data
  const { handleMinus, handleChange, handlePlus, __2 } = _instance
  return _xc.h(_xc.XbenchcView, {'style': _xc.s("b-view")}, _xc.h(_xc.XbenchcView, {'onClick': handleMinus,'style': _xc.s("b-view-minus")}, _xc.h(_xc.XbenchcText, null, "−")), _xc.h(_xc.XbenchcInput, {'value': value,'onChange': handleChange,'style': _xc.s("b-view-input")}), _xc.h(_xc.XbenchcView, {'onClick': handlePlus,'style': _xc.s("b-view-plus")}, _xc.h(_xc.XbenchcText, null, "+")))
}

export default function Xbenchc168617(props) {
