...
const render = (_props, _instance, _xc, children) => {
  const { fop, image, containerStyle, slotStyle, __0 } = _props
  const { __1 } = _instance.data
  const { __2 } = _instance
  return _xc.h(_xc.XbenchcView, {'className': "b-view",'style': _xc.s("b-view").concat({...(containerStyle)})}, _xc.h(_xc.XbenchcImage, {'className': "b-image",'src': dwtools.qiniu(image, fop),'style': _xc.s("b-image").concat({width: '100%', height: '100%'})}), _xc.h(_xc.XbenchcView, {'className': "b-slot",'style': _xc.s("b-slot").concat({...(slotStyle)})}, children))
}

export default function Xbenchc184617(props) {
