...
const render = (_props, _instance, _xc, children) => {
  const { value, eventData, eventData2, imageWidth, imageHeight, trueImage, falseImage, __0 } = _props
  const { __1 } = _instance.data
  const { handleClick, __2 } = _instance
  return _xc.h(_xc.XbenchcView, {'onClick': handleClick,'style': _xc.s('b-view' + (value ? ' b-view-active' : ''))}, ((value) && (_xc.h(_xc.XbenchcImage, {'className': "b-image",'style': _xc.s("b-image").concat({width: dwtools.pm(imageWidth), height: dwtools.pm(imageHeight)}),'src': dwtools.qiniu(trueImage, 'wh,50,50')}))), ((!value) && (_xc.h(_xc.XbenchcImage, {'className': "b-image",'style': _xc.s("b-image").concat({width: dwtools.pm(imageWidth), height: dwtools.pm(imageHeight)}),'src': dwtools.qiniu(falseImage, 'wh,50,50')}))))
}

export default function Xbenchc168616(props) {
