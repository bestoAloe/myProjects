...
const render = (_props, _instance, _xc, children) => {
  const { fop, value, width, height, image, __0 } = _props
  const { __1 } = _instance.data
  const { __2 } = _instance
  return _xc.h(_xc.XbenchcView, {'className': "b-view",'style': _xc.s("b-view").concat({width: dwtools.pm(width), height: dwtools.pm(height)})}, _xc.h(_xc.XbenchcImage, {'className': "b-image",'src': dwtools.qiniu(image, fop),'style': _xc.s("b-image").concat({width: dwtools.pm(width), height: dwtools.pm(height)})}), _xc.h(_xc.XbenchcView, {'style': _xc.s("b-slot")}, children))
}

export default function Xbenchc171194(props) {
