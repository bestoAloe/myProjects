...
const render = (_props, _instance, _xc, children) => {
  const { value, eventData, eventData2, bgColor, activeBgColor, __0 } = _props
  const { __1 } = _instance.data
  const { handleClick, __2 } = _instance
  return _xc.h(_xc.XbenchcView, {'className': 'b-view' + (value ? ' b-view-active' : ''),'style': _xc.s('b-view' + (value ? ' b-view-active' : '')).concat({backgroundColor: value ? activeBgColor: bgColor}),'onClick': handleClick}, _xc.h(_xc.XbenchcView, {'style': _xc.s('b-node' + (value ? ' b-node-active' : ''))}))
}

export default function Xbenchc168615(props) {
