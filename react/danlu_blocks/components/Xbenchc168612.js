...
const render = (_props, _instance, _xc, children) => {
  const { backgroundColor, __0 } = _props
  const { height, __1 } = _instance.data
  const { __2 } = _instance
  return _xc.h(_xc.XbenchcView, {'className': "b-view",'style': _xc.s("b-view").concat({backgroundColor: backgroundColor, height: height})})
}

export default function Xbenchc168612(props) {
